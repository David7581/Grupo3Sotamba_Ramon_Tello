...
try
    ...
    fprintf("\nEjemplo 1\n");
    [f] = diffinitasatras(y1,x0, p); 
 
    for i = 1:4
        y1 = diff(y1);
        r1 = subs(y1 ,x ,x0);
        error_relativo1 = abs(r1 - f(i)) * 100 / r1;
        fprintf("\nDerivada %i en %f es: %f\n", i, x0, f(1,i)); % Imprime el valor de la derivda en x0
        fprintf("\nEl error en la derivada %i es de; %f %%\n", i, error_relativo1); % Imprime el valor de error de la derivada en x0
    end
catch e
    fprintf("Error: %s\n",e.message);
end
...
try
    ...
    for i = 1:4
        y1 = diff(y1);
        r1 = subs(y1 ,x ,x0);
        error_relativo1 = abs((r1 - f(i))/ r1) *100 ;
        fprintf("\nDerivada %i en %f es: %f\n", i, x0, f(1,i)); % Imprime el valor de la derivda en x0
        fprintf("\nEl error en la derivada %i es de; %f %%\n", i, error_relativo1); % Imprime el valor de error de la derivada en x0
    end
catch e
